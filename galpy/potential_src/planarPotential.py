...
class planarPotential(object):
    ...
    def _Rforce_nodecorator(self,R,phi=0.,t=0.,v=None):
        # Separate, so it can be used during orbit integration
        try:
...
class planarPotentialFromRZPotential(planarAxiPotential):
    ...
    def _Rforce(self,R,phi=0.,t=0.,v=None):
        ...
        HISTORY:
           2010-07-13 - Written - Bovy (NYU)
        """
        return self._Pot.Rforce(R,0.,t=t,v=v,use_physical=False)
...
class planarPotentialFromFullPotential(planarPotential):
    ...
    def _Rforce(self,R,phi=0.,t=0.,v=None):
        ...
        HISTORY:
           2016-06-02 - Written - Bovy (UofT)
        """
        return self._Pot.Rforce(R,0.,phi=phi,t=t,v=v,use_physical=False)
    ...
    def _phiforce(self,R,phi=0.,t=0.,v=None):
        ...
        HISTORY:
           2016-06-02 - Written - Bovy (UofT)
        """
        return self._Pot.phiforce(R,0.,phi=phi,t=t,v=v,use_physical=False)
...
@potential_physical_input
@physical_conversion('force',pop=True)
def evaluateplanarRforces(Pot,R,phi=None,t=0.,v=None):
    """
    NAME:
...
def _evaluateplanarRforces(Pot,R,phi=None,t=0.,v=None):
    ...
    if isinstance(Pot,list) \
        ...
        for pot in Pot:
            if nonAxi:
                ...
                sum+= pot._Rforce_nodecorator(R,t=t,v=v)
        return sum
    elif isinstance(Pot,planarPotential):
        if nonAxi:
            return Pot._Rforce_nodecorator(R,phi=phi,t=t,v=v)
        else:
            return Pot._Rforce_nodecorator(R,t=t,v=v)
    else: #pragma: no cover 
        raise PotentialError("Input to 'evaluatePotentials' is neither a Potential-instance or a list of such instances")

@potential_physical_input
@physical_conversion('force',pop=True)
def evaluateplanarphiforces(Pot,R,phi=None,t=0.,v=None):
    """
    NAME:
    ...
    """
    return _evaluateplanarphiforces(Pot,R,phi=phi,t=t,v=v)

def _evaluateplanarphiforces(Pot,R,phi=None,t=0.,v=None):
    from galpy.potential_src.Potential import _isNonAxi
    isList= isinstance(Pot,list)
    nonAxi= _isNonAxi(Pot)
    if nonAxi and phi is None:
    ...
    if isinstance(Pot,list) \
        ...
        for pot in Pot:
            if nonAxi:
                sum+= pot._phiforce_nodecorator(R,phi=phi,t=t,v=v)
            else:
                sum+= pot._phiforce_nodecorator(R,t=t,v=v)
        return sum
    elif isinstance(Pot,planarPotential):
        if nonAxi:
            return Pot._phiforce_nodecorator(R,phi=phi,t=t,v=v)
        else:
            return Pot._phiforce_nodecorator(R,t=t,v=v)
    else: #pragma: no cover 
        raise PotentialError("Input to 'evaluatePotentials' is neither a Potential-instance or a list of such instances")
